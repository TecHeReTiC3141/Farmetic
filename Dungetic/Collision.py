...
map_image = pygame.transform.scale(map_image, (
    int(map_image.get_width() / 2.5), int(map_image.get_height() * 0.6))).convert_alpha()
...
class Room:
    ...
    def __init__(self, walls_list, entities_list, entrances):
...
for i in range(1, dung_width * dung_length + 1):
    ways = random.sample(directions, random.randint(2, 4))
    enters = []
    walls = []
    if 'up' in ways and i > dung_length:
        walls.append(Wall(0, 0, width=random.randint(dung_width // 2 - 250, dung_width // 2 - 100), height=random.randint(50, 100)))
        walls.append(Wall(x := random.randint(display_width // 2 + 150, display_width // 2 + 220), y := 0, width := display_width - x, height := random.randint(50, 100)))
        enters.append('up')
    else:
        walls.append(Wall(0, 0, width := display_width, height := random.randint(50, 100)))

    if 'down' in ways and i < (dung_width - 1) * dung_length:
        walls.append(Wall(0, y := random.randint(display_height - 300, display_height - 250),
                          width=random.randint(dung_width // 2 - 250, dung_width // 2 - 100), height=display_height - y))
        walls.append(
            Wall(x := random.randint(display_width // 2 + 150, display_width // 2 + 220), y := random.randint(700, 750), width=1000 - x, height=800 - y))
        enters.append('down')
    else:
        walls.append(Wall(0, y := random.randint(700, 750), width := 1000, height := 800 - y))

    if 'left' in ways and i % dung_length != 1:
        walls.append(Wall(0, 0, width=random.randint(50, 100), height=random.randint(250, 400)))
        walls.append(Wall(0, y := random.randint(650, 720), width := random.randint(50, 100), height := 800 - y))
        enters.append('left')
    else:
        walls.append(Wall(0, 0, width=random.randint(50, 100), height=1000))

    if 'right' in ways and i % dung_length:
        walls.append(Wall(x := random.randint(900, 950), 0, width=1000 - x, height=random.randint(250, 400)))
        walls.append(Wall(x := random.randint(900, 950), y := random.randint(650, 720), width=1000 - x, height=800 - y))
        enters.append('right')
    else:
        walls.append(Wall(x := random.randint(900, 950), y := 0, width := 1000 - x, height := 800))
    ...
    rooms[i] = Room(walls, entities_list, enters)
...
while True:
    ...
    if not current_interface:
        ...
        elif keys[pygame.K_d] and heretic.x < 925 and not right_stop:
        ...
        elif keys[pygame.K_s] and heretic.y < 700 and not down_stop:
...
Расчет столкновений еретика со стеной
    ...
    if heretic.y < 20 and curr_room > dung_length:
        ...
        heretic.y = 670
    elif heretic.y > display_height - 10 and curr_room < dung_width * (dung_length - 1):
        curr_room += dung_length
        heretic.y = 30
